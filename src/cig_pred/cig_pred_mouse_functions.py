...
def pesudobulk_cigpred_mouse (inputtype_pred,exp_filename_pred,alll_seq_features1,gene_name_length1,training_table_mouse):
    ...
    training_table_mouse=training_table_mouse
    ...
    if inputtype_pred =='rawcount':
        print("Unormalised expression value rawcounts are processing")
        ...
        print("SKIPPING the rawcount processing, Since the expression values are already in TPM")
    ...
    for i in ten_celltype_train:
        print("CIG-Pred: Identifying the CIGs in ",i)
        ...
        combine_feat = combine_feat[['Geneid','TPM_gene_exp_mean', 'mca3gini', 'mtdtau_log',
               'mtdroku_specificity_log', 'Total_utr_5_len', 'tss_dis',
               'TF_motif_meanexon', 'TF_motif_cv_tss_3kb',
        ...
        ##cig_pred trained model  mouse
        X_train = training_table_mouse
        X_train = X_train.drop(columns=['label', 'Gene_name'])
        Y_train = training_table_mouse['label']
...
def pesudobulk_cig_reg_pred_mouse (cig_pred_result,all_db_grn_mouse,tf_mouse,training_table_cigreg_mouse):
    ...
    for c_type in celltype:
        print("CIG_reg-Pred: Identifying the  Master TF of CIGs in ",c_type)
        c_type1=c_type
        ...
        mas_tf_cig_exp_list=[]
        master_tf_cigscore=pd.merge(cig_score_gene_exp,tf_mouse,left_on=['symbol'],right_on=['TF'],how='inner')
        ...
        master_tf_parent_edge=pd.merge(tf_mouse,all_db_grn,left_on=['TF'],right_on=['target_genename'],how='inner')
        ...
        master_tf_child_edge=pd.merge(tf_mouse,all_db_grn,left_on=['TF'],right_on=['source_genename'],how='inner')
        ...
        master_tf_cig_final=pd.merge(tf_mouse,master_tf_child_parent_edge,on=['TF'],how='left')
        ...
        ##cig_reg-pred trained model  mouse
        X_train = training_table_cigreg_mouse
        X_train = X_train.drop(columns=['label'])
        Y_train = training_table_cigreg_mouse['label']
...
def cig_pred_singlecell(all_seq_features1,cell_ranger_file_name,training_table_mouse):    
    ...
    counts_mat = io.mmread(str(cell_ranger_file_name)+"matrix.mtx")
    ...
    gene_length = pd.read_csv("./Generic_features_GTF_gene_with_features_prot_cds_BO_dataset.txt",sep="\t") 
    ...
    ##REQ features
    combine_feat=all_seq_features1[['Geneid','mca3gini', 'mtdtau_log',
           'mtdroku_specificity_log', 'Total_utr_5_len', 'tss_dis',
           'TF_motif_meanexon', 'TF_motif_cv_tss_3kb',
           'TF_motif_totaltss_2kb_norm_GL', 'utr5_mirna_count',
           'exon_avg_CS_median', 'R_codon_bias_median_trans',
    ...
    for i in cells_list:
        ...
        test_file_model=test_file_model[['Geneid','TPM_gene_exp_mean', 'mca3gini', 'mtdtau_log',
               'mtdroku_specificity_log', 'Total_utr_5_len', 'tss_dis',
               'TF_motif_meanexon', 'TF_motif_cv_tss_3kb',
               'TF_motif_totaltss_2kb_norm_GL', 'utr5_mirna_count',
               'exon_avg_CS_median', 'R_codon_bias_median_trans',
    ...
    #desc_fun_pred_cig_list=pd.merge( pro_cds_gens_names_m,desc_fun_pred_cig_list, on=["Geneid"], how="inner")
    
    #desc_fun_pred_cig_df = pd.DataFrame(desc_fun_pred_cig)
    #desc_fun_pred_cig_df.to_csv(r'./all_CIG_list_all_cells.txt',index = False,sep=",")
    #desc_fun_pred_cig_df.columns=['gene_symbols']
    #desc_fun_pred_cig_df1=desc_fun_pred_cig_df['gene_symbols'].value_counts()
    #desc_fun_pred_cig_df1.to_csv(r'./all_CIG_list_all_cells_req.txt',sep=",")  
